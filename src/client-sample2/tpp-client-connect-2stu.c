...
void create_ACK_h(struct tpphdr *p)
{
	...
	set_seg_seq_h(p, g_client_tcb.snd_nxt);
...
int client_fsm(void *buf, size_t len)
{
	...
	case TPPS_SYN_SENT:
		...
		// update tcb
		g_client_tcb.snd_nxt = g_client_tcb.iss+1; // according to Kuross Ross book
							   // but some implementation may not +1
 ...
 */
int process_segment_client(void *buf, size_t len, struct sockaddr_in *p_local, struct sockaddr_in *p_remote) {

	// verify connection Identity
	if (!lookup_tppcb(&g_client_tcb, 1, p_local, p_remote)) {
		fprintf(stderr, "unknown sender!\n");
		return -1;
	}
...
int tpp_connect()
{
	...
	printf("Recvd %d bytes from %s %d.\n",
		(int)recvlen, inet_ntoa(sender.sin_addr), ntohs(sender.sin_port));
	fflush(stdout);
#endif // _DEBUG_
	...
	return (process_segment_client(buf, recvlen, p_client, &sender));
}
...
int main(int argc, char *argv[]) {
	...
	p_client->sin_addr.s_addr = htonl(INADDR_ANY);
	/*
